...
核对日期：2026-09-17；当前代码审查候选 `683780c`（包含只读预设空权限模式修复、SEC-001 作用域过滤和本轮文档收口）。最新真实 Provider 结果绑定前一代码候选 `66c57fd`；`683780c` 新增权限实现后尚未重新绑定，历史发布候选 `2a86886` 及更早记录继续保留。任务状态只在 [TASKS](TASKS.md) 维护。
...
| 发布准备 | 五平台源码测试、候选 GoReleaser dry-run、五平台发布归档 Smoke、linux/amd64+arm64 Docker Smoke、前一候选真实 Provider 记录 | B02/B03/B04/B06 已修复；`66c57fd` 已完成 OpenAI-compatible 连续 3 次真实通过和全量 CI，当前 `683780c` 因新增 SEC-001 需重新绑定，B05 仍缺不同协议/入口证据 |
