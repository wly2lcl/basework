...
核对日期：2026-09-17；当前代码审查候选 `1675f7a`（包含只读预设空权限模式修复、SEC-001 作用域过滤、在途检查上下文快照、精确参数缓存匹配和审计归属，以及 SQLite 参数缓存回归）。最新真实 Provider 结果绑定前一代码候选 `66c57fd`；`1675f7a` 新增权限缓存/审计边界后尚未重新绑定，历史发布候选 `2a86886` 及更早记录继续保留。任务状态只在 [TASKS](TASKS.md) 维护。
...
| 发布准备 | 五平台源码测试、候选 GoReleaser dry-run、五平台发布归档 Smoke、linux/amd64+arm64 Docker Smoke、前一候选真实 Provider 记录 | B02/B03/B04/B06 已修复；`66c57fd` 已完成 OpenAI-compatible 连续 3 次真实通过和全量 CI，当前 `1675f7a` 因新增权限缓存/审计边界需重新绑定，B05 仍缺不同协议/入口证据；该候选尚未推送 |
