...
建议第一次在测试项目里依次验证：读一个文件、生成一个小函数、运行对应测试、退出后查询持久化记录；需要继续时，用 `basework session list` 找到 ID，再以 `basework tui --session <id>` 启动或在 TUI 输入 `/session <id>`。调用成功与完成一次真实编程任务是不同的验证层次。
