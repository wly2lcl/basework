...
由 [pkg/agent](agent.md) 的 `BuildRequestMessages` 在组装请求时放到消息列表最前；
后者只留请求指纹。它们留在投影之外有两个硬理由——消息列表的 Seq 必须保持非递减
（压缩锚点依赖），且 system 消息必须在最前而压缩只截断尾部。
