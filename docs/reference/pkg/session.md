...
- **JSONL 追加是增量写入**：`AppendEvent` 在会话文件锁内用 `O_APPEND` 写入单行，并按
  64 条批量 `Sync`；只在检测到外部实例改动或旧版本日志时回读/迁移。崩溃留下的最后
  一条不完整行会在读取时跳过、下次追加时修剪；中间损坏仍显式报错。长会话性能与
  SQLite 对照见 [OPT-003](../../development/evidence/OPT-003.md)。
