...
### 长会话 JSONL 追加优化（2026-09-16）

- `JSONLStore.AppendEvent` 改为文件锁内的增量 `O_APPEND` 单行写入，避免每条事件重写整份历史；
  跨实例追加会检测外部变化并刷新缓存，旧版本日志仍按原迁移链处理。
- 增加有界批量 `Sync`、崩溃残尾修剪和中间损坏拒绝；旧日志、跨实例序号、双压缩重启与默认/完整
  tags 回归均通过。10000 事件同条件追加中位数约 1.07 秒，详见 [OPT-003 证据](development/evidence/OPT-003.md)。
