...
- [x] 关闭后不再进入 Agent；取消排队请求及时返回且无副作用。
- [x] 所有等待者和订阅者都结束，资源只释放一次；终态、错误与持久化记录一致。
...
- [x] 启用的行为真正出现在主 Agent 并触发相应结果，默认行为无意外回退。
...
修复候选 `3ad67a7d30ab5b030bd28fe09504b6134c5a6faf` 的默认/完整 tags 全量测试、相关
race 与 `tests/tui_pty/run_all.py` scenario1–4 均通过。scenario1 覆盖 CLI/TUI 统一服务
与取消，scenario2 覆盖后台任务取消和进程组清理，scenario3 覆盖硬杀恢复与旧回调隔离，
scenario4 覆盖模型能力和缺 key 错误。RUN-003 依赖回验完成。
