...
- [x] 从空数据目录真实读→改→跑→关闭→重启，事实存在且不重复；不直接调用 SaveWorkspaceFacts 准备成功前提。
- [x] 部分成功、失败原因、不同工作区、同文件多次变化均有可追溯结果。
...
- [x] 80 字节等小预算下最终输出不超限；超大文件不会触发无界读取。
- [x] 事实文件变化能在产品下一次请求中出现；来源明确，过期事实不被当作当前文件状态。
...
- [x] 重启前后选中的 session ID 一致，模型请求确含旧事实；丢失旧历史或关闭压缩后测试必须失败。
...
- [x] 前置任务全部重新完成；只复跑本卡受影响的真实入口/取消/恢复验收，不重写已通过模块。
- [x] 证据记录修复后候选 commit 与实际结果；发布任务同时更新 CI/归档/真实 Provider 对应关系。

### 2026-09-16 修复候选回验

修复候选 `3ad67a7d30ab5b030bd28fe09504b6134c5a6faf` 已在 `tests/tui_pty/run_all.py` 的
scenario3（硬杀后同 ID 恢复）和 scenario8（双压缩后同 ID 继续）通过；默认/完整 tags
全量测试及相关 race 通过。恢复请求实际携带历史唯一标记，遗留任务为 `interrupted`，
没有自动重跑或重复会话。CTX-003 依赖回验完成，真人主观 TUI 手感仍按发布边界单独记录。
