...
- [ ] 合成秘密不会出现在工具返回模型的消息、结果 JSON 或 stdout/stderr 中；工具/测试子进程不通过继承环境获得 Provider key。
- [ ] 错误 Add + 删除/跳过断言、改模块、只加注释均验收失败；正确实现通过，原始测试哈希完整且预期测试实际执行。
- [ ] 超时/大输出/派生子进程夹具在短预算内终止，进程无残留、输出有界、失败结果文件存在且已脱敏。
- [ ] 默认和完整 tags 的 CI 都实际运行；不能拿架构测试替代默认全量测试。
- [ ] 两个 Provider 探针转正式回归；相关默认/完整测试通过，真实结果按 SHIP-001 原要求补证。
...
- [ ] 与 OPT-002 同条件三次样本，10000 事件总追加耗时中位数 ≤10 秒且较重测基线至少快 10 倍；累计分配 ≤1 GB。
- [ ] 1000→10000 事件耗时增长 ≤20 倍，避免仍然是二次重写；若目标不可达先记录 profile 与方案决定，不自行放宽门槛后标完成。
- [ ] 旧日志/锁/崩溃恢复/双压缩回归均通过，默认与完整 tags 兼容；用户数据不被静默迁移或丢弃。
- [ ] 受影响平台存储测试及三次原始基准结果可追溯，代码完成不等于性能验收通过。
