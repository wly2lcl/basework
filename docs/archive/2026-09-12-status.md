...
与参考项目的横向对比见 [ROADMAP.md](2026-09-12-roadmap.md#与参考项目对比)。
...
| `pkg/session/migrate.go` | ✅ 完成（2026-09-12） | 会话格式版本化 + **只允许相邻步进**的迁移链；读到更高版本返回 `ErrSchemaTooNew` 并拒写 |
| `pkg/session/projection.go` | ✅ 完成（2026-09-12） | 压缩锚点由消息下标改为事件 Seq；`ProjectMessagesWithSeq` 返回每条消息的来源 Seq |
...
| 模块 | 状态 | 说明 |
|------|------|------|
| `scripts/docstats` + `make stats` | ✅ 完成 | 生成 `docs/STATS.md`（代码规模/测试/文件数），终结手写数字漂移；**不含覆盖率**（不可复现），覆盖率见 `make coverage` |
| `make coverage` | ✅ 完成（2026-09-12） | `go test -cover` 仅打印、不写文件、不入门禁（覆盖率对含并发的包本身不可复现） |
| `scripts/gendeps` + `make deps` | ✅ 完成（2026-09-12） | 生成 `docs/DEPGRAPH.md`；**构建约束感知**的依赖图 + 层级违规检测 |
| `scripts/doccheck` + `make check-docs` | ✅ 完成（2026-09-12） | ADR 结构校验 + 全仓 markdown 相对链接校验 |
| `tests/arch_test.go` | ✅ 完成（2026-09-12） | 三条架构规则：`pkg↛internal/cmd`、`internal↛cmd`、`Set*` 注入点白名单 |
| `docs/adr/` | ✅ 完成（2026-09-12） | 轻量 ADR（仅「背景/决定/后果」三节），已回填 0001–0003 |
| CI 新鲜度门禁 | ✅ 完成（2026-09-12） | `make gen` 后 `git diff --exit-code docs/DEPGRAPH.md docs/STATS.md` |
| 请求可审计（"模型可见即已记录"） | ✅ 完成（2026-09-12） | `agent.BuildRequestMessages` 使请求成为事件日志的纯函数；`EventRequestBuilt` 留请求指纹；`agent.CheckRequestInvariant` 可校验等式 |
| `pkg/` 包 README 契约 | ✅ 完成（2026-09-12） | 12 个包四节齐备（用途/配置/扩展点/Known Limitations），由 `scripts/doccheck` 机检 |
...
[ROADMAP.md](2026-09-12-roadmap.md#与参考项目对比)，避免两处各有一份而口径漂移。
