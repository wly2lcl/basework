...
>
> Phase 1-12：核心框架（已完成 ✅）| Phase 13-25：终端产品化（已完成 ✅）| Phase 26-30：稳定性、CI/CD、TUI 基础和生态（已完成 ✅）| Phase 35：主路径收口（已完成 ✅）| Phase 36：核心能力追赶 crush（规划中）| Phase 37：治理基建（已完成 ✅）
>
...
## Phase 37: 治理基建（借鉴 deepseek-harness）（2026-09-12 完成 ✅）

**背景**：对照 `deepseek-ai/deepseek-harness` 的对比分析确认，basework 在「会话数据的向后兼容」与「架构约束的机器化」两块地基上此前是缺位的：事件格式没有版本号，一旦结构调整就会静默污染历史会话；依赖边界只写在文档里，无人校验。本阶段不新增功能，只补这两类地基。方案与取舍见 [plans/2026-09-12-dsh-borrow.md](2026-09-12-dsh-borrow.md)。

**范围说明**：第一轮执行方案中的 A（会话格式版本化）、C（依赖图 + 新鲜度门禁）、D（轻量 ADR）；第二轮（单独授权）补齐 B（"模型可见即已记录"不变量）与 E（`pkg/` 包 README 契约）。至此方案 §5 全部落地。

### Task 37.1: 会话格式版本化 + 相邻迁移链（A1-A4）✅

**文件**：`pkg/session/event.go`, `pkg/session/migrate.go`, `pkg/session/jsonl.go`, `pkg/session/sqlite.go`, `pkg/session/migrate_test.go`, `pkg/session/migrate_sqlite_test.go`

**内容**：
- `Event.SchemaVersion`（JSON key `v,omitempty`），零值即 v0（兼容存量数据）
- JSONL 文件头 `{"_schema":"basework.session","v":1}`；SQLite 用 `PRAGMA user_version`
- 迁移链只允许相邻步进（vN→vN+1），跳版或回退直接报错
- 读到比当前更高的版本返回 `ErrSchemaTooNew`，且**读路径与写路径都必须拒绝**

**验收标准**：
- [x] 无版本号的存量 JSONL 可被读取并就地补写文件头（v0→v1）
- [x] 伪造 `v:99` 的文件在读取时被拒，且不会被继续追加写入
- [x] 迁移链拒绝跨版本跳跃
- [x] SQLite 库版本高于当前时打开即失败

### Task 37.2: 压缩锚点稳定化 + 摘要回填（A5）✅

**文件**：`pkg/session/projection.go`, `pkg/agent/loop.go`, `pkg/agent/interfaces.go`, `internal/compaction/*`, `pkg/agent/compaction_wiring_test.go`, `pkg/session/projection_compact_test.go`

**内容**：
- `EventCompacted` 的截断锚点由消息**下标**（`KeepFrom`）改为事件 **Seq**（`TruncatedSeq`），下标会随投影规则变化而漂移
- 摘要文本不再丢失，随事件落盘并在重建历史时插回（前缀常量 `SummaryMessagePrefix`）
- 通过**可选接口** `agent.CompactReporter` 传递摘要，`agent.Compactor` 保持不变 → **零公开 API 破坏**

**验收标准**：
- [x] 投影结果在 schema 迁移前后完全一致
- [x] `TruncatedSeq` 优先于陈旧的 `KeepFrom`
- [x] 摘要随历史插回；缺失 Seq 时回退旧锚点
- [x] 二次压缩正确取代首次压缩（不出现两条摘要）
- [x] `Compactor` 公开接口未变，旧实现走回退路径

### Task 37.3: 依赖图生成 + 新鲜度门禁（C1-C3）✅

**文件**：`scripts/gendeps/main.go`, `docs/DEPGRAPH.md`, `tests/arch_test.go`, `Makefile`, `.github/workflows/build.yml`

**内容**：
- `scripts/gendeps` 纯标准库扫描 `pkg/`、`internal/`、`cmd/`，**感知构建约束**（`//go:build`），区分"默认构建"与"标签门控"导入
- 输出 `docs/DEPGRAPH.md`：层级概览、mermaid 图、边清单、fan-in/fan-out、违规检测、`pkg/` 三方依赖明细；**有违规即非零退出**
- `scripts/docstats` 去掉时间戳 → 生成物可复现
- CI 增加 `make gen` 后 `git diff --exit-code` 新鲜度门禁

**验收标准**：
- [x] `make check-arch` 跑 `TestArch` 全绿
- [x] `docs/DEPGRAPH.md` 无层级违规
- [x] 生成物可复现（无时间戳），重复生成 diff 为空

### Task 37.4: 轻量 ADR + 文档校验器（D1-D3）✅

**文件**：`docs/adr/`, `scripts/doccheck/main.go`, `Makefile`

**内容**：
- ADR 只保留「背景 / 决定 / 后果」三节 + `Status`，不引入流程
- `scripts/doccheck` 校验 ADR 文件名（`NNNN-kebab-title.md`）与三节结构，并校验全仓 markdown 相对链接有效性
- 回填 0001（记录架构决策）、0002（解耦 `pkg/` 与 `internal/`）、0003（文档单一事实源）

**验收标准**：
- [x] `make check-docs` 通过；故意写坏链接/缺节会被拒
- [x] `docs/adr/` 三个 ADR 结构合法

### Task 37.5: 「模型可见即已记录」不变量（B）✅

**文件**：`pkg/session/event.go`, `pkg/agent/request.go`, `pkg/agent/pipeline.go`, `pkg/agent/loop.go`, `pkg/agent/request_invariant_test.go`

**内容**：
- 新增 `EventSystemPromptSet` / `EventSteered`，把此前绕过日志的两条路径（配置里的 system prompt、`Drain()` 后即丢的 steering）落成事件
- 新增 `EventRequestBuilt`：每次真实请求留一条指纹（条数 + 哈希 + 来源）
- `agent.BuildRequestMessages(events)` 成为请求的唯一组装入口——请求是事件日志的**纯函数**
- 新增 `agent.CheckRequestInvariant(events, actual)` 做 strict 校验
- `SchemaVersion` 升到 2（v1→v2 为纯版本戳；理由是能力门槛，不是数据变换）

**验收标准**：
- [x] 系统提示与 steering 可从事件日志重建，且顺序为 system → steering → 历史
- [x] 多轮对话中每轮都满足「实际请求 == 日志重建的请求」
- [x] `request.built` 的哈希与实际请求一致（含 `nil`/空切片归一化）
- [x] 只改配置、不落事件的 system prompt **不进入**请求
- [x] `CheckRequestInvariant` 能检出长度差异与内容改写
- [x] v1 日志可升到 v2，投影结果不变

### Task 37.6: `pkg/` 包 README 契约（E）✅

**文件**：`pkg/*/README.md`（12 个）, `scripts/doccheck/main.go`

**内容**：
- 12 个 `pkg` 下的 Go 包各补一份 README，统一四节：用途 / 配置 / 扩展点 / Known Limitations
- 契约并入 `scripts/doccheck`：缺 README 或四节不全即非零退出

**验收标准**：
- [x] `make check-docs` 通过，并报出「pkg 包 README 12 个」
- [x] 删掉任一包 README → 被拒（退出码 1）
- [x] 去掉任一节 → 被拒（退出码 1）
- [x] `pkg` 根目录（仅集成测试）与 `pkg/agent/templates` 不误报

**对 Phase 36 的影响**：`36.5`（backend 分层）所需的架构边界前提（依赖图 + 结构断言）已在本阶段补齐。
