...
> 可执行的细化待办见 [docs/TASKS.md](../TASKS.md)。
...
| 代码规模 / 测试 | 见 [docs/STATS.md](../STATS.md)，由 `make stats` 自动生成 |
...
2026-09-12 完成。对照 deepseek-harness 补三类此前确实缺失的能力，不新增功能，只补
「数据安全」与「制度」两类地基。方案与取舍见
[docs/plans/2026-09-12-dsh-borrow.md](2026-09-12-dsh-borrow.md)。

| 项 | 内容 | 产出 |
|---|---|---|
| 会话格式版本化 | 事件与 JSONL 文件头带版本号；SQLite 用 `PRAGMA user_version` | `pkg/session/migrate.go` |
| 相邻迁移链 | 只允许 v0→v1→v2 步进；读到更高版本返回 `ErrSchemaTooNew` 并拒写 | 同上 |
| 压缩锚点稳定化 | `EventCompacted` 由消息**下标**改为事件 **Seq** 锚点 | `pkg/session/projection.go` |
| 压缩摘要回填 | 摘要不再被丢弃，随事件落盘并插回历史 | `agent.CompactReporter` |
| 依赖图 + 新鲜度门禁 | 结构断言交给机器生成与 CI 校验 | `scripts/gendeps`、`docs/DEPGRAPH.md` |
| 文档校验器 | ADR 结构 + 全仓 markdown 相对链接 | `scripts/doccheck`、`make check-docs` |
| 轻量 ADR | 只允许「背景/决定/后果」三节 | `docs/adr/`（0001–0003） |
| 请求可审计 | 请求成为事件日志的纯函数；每次请求留指纹 | `agent.BuildRequestMessages`、`EventRequestBuilt` |
| `pkg/` README 契约 | 12 个包四节齐备，并入机检 | `pkg/*/README.md`、`scripts/doccheck` |

对 Phase 36 的影响：`36.5`（backend 分层）所需的架构边界前提已在本次补齐。
...
验收标准见 [docs/TASKS.md](../TASKS.md) 的 Phase 36 章节。
...
- **文档一致性**：配置 schema、CLI 行为、默认模型与发布方式保持同步。
  结构与规模类数字**不手工维护**——由 `make gen` 生成、CI 做新鲜度门禁；
  `make check-docs` 校验 ADR 结构与全仓链接
...
`TestArch*`（`tests/arch_test.go`）强制守护，CI 中执行；`make deps` 生成的
[docs/DEPGRAPH.md](../DEPGRAPH.md) 会同时报告违规。
...
`pkg/` 层以标准库为主，第三方依赖限定在白名单内。**当前全部清单以
[docs/DEPGRAPH.md](../DEPGRAPH.md) 为准**（`make deps` 生成，CI 校验新鲜度）：
`golang.org/x/image`、`golang.org/x/oauth2`、`modernc.org/sqlite`、
`github.com/golang/snappy`。其余第三方依赖出现在终端产品（`internal/`、`cmd/`）。

> 本节此前写「除 `golang.org/x/image` 外不依赖第三方库」，与代码不符（少列了 3 个）。
> 结构性断言和数字一样会漂移，现在改由依赖图生成器做机器核对。
