...
- 2026-09-18 在当前 `main=origin/main=0fd6101` 重新通过 `make check-docs`、`make gen`、`git diff --check`、默认/`sqlite memory` 全量测试和关键 race 测试；提升权限用于允许本地临时端口后，OAuth、pprof、webfetch、Provider runner 相关测试均通过。文档提交 `0fd6101` 的 CI [35294261264](https://github.com/wly2lcl/basework/actions/runs/35294261264) 也已全绿。此次复核只更新门禁证据，不改变任务状态。
- 当前候选边界：`a5126c8` 是包含 MCP/runner 修复的可执行代码候选，`0ddb0ac`/`0ab9700`/`0fd6101` 是其后的文档提交；此前 `a78efba` 的三份真实 Provider JSON 不能证明 `a5126c8`，必须重新绑定当前 runner。
