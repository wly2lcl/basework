...
| LOAD-001 | M8 | [真实负载与稳定性验证](tasks/09-follow-up.md#load-001) | 完成 | RESP-001 | Codex / 2026-09-18 Agnes 合成负载 5 次连续 + 3 路并发；多 key 6 路无 429、12 路超时边界 | [记录](development/evidence/LOAD-001.md) |
...
- `0b8fe97` 补齐 runner 的代理 query/fragment 脱敏、最终 stderr API key 清洗和独立验证 `WaitDelay=2s` 管道等待上界；代理 token/fragment 回归、默认/SQLite 全量、关键 race 和主 CI [35200027697](https://github.com/wly2lcl/basework/actions/runs/35200027697) 均通过。该 runner 变化仍需在当前候选重新生成真实 Provider artifact。
- `ceed232` 修复 `pkg/mcp/StdioTransport.Call` 的已取消 context 竞态：避免已取消请求写入，并让取消在响应同时就绪时优先。`TestStdioCallWithContextCancel` 重复 100 次、MCP 全包、默认/SQLite 全量和主 CI [35200027697](https://github.com/wly2lcl/basework/actions/runs/35200027697) 均通过。
- 文档提交 `69b9c34` 的主 CI [35201403018](https://github.com/wly2lcl/basework/actions/runs/35201403018) 暴露 macOS `TestIndependentTestHonorsTimeoutAndBoundsOutput` 在夹具 `.basework-home` 上的清理竞态。提交 `a5126c8` 将独立验证 HOME 移到仓库外临时目录，并增加有界清理重试；超时回归重复 50 次、默认/SQLite 全量、关键 race 和主 CI [35202387330](https://github.com/wly2lcl/basework/actions/runs/35202387330) 均通过。
- 2026-09-18 在当前已同步工作树重新通过 `make check-docs`、`make gen`、`git diff --check`、默认/`sqlite memory` 全量测试和关键 race 测试；提升权限用于允许本地临时端口后，OAuth、pprof、webfetch、Provider runner 相关测试均通过。文档提交 `0fd6101` 的 CI [35294261264](https://github.com/wly2lcl/basework/actions/runs/35294261264)、`3685a4d` 的 CI [35295142531](https://github.com/wly2lcl/basework/actions/runs/35295142531) 与 `4cd915b` 的 CI [35296064488](https://github.com/wly2lcl/basework/actions/runs/35296064488) 均已全绿。此次复核只更新门禁证据，不改变任务状态。
- 当前候选边界：`a5126c8` 是包含 MCP/runner 修复的可执行代码候选，`0ddb0ac`/`0ab9700`/`0fd6101`/`3685a4d`/`4cd915b` 是其后的文档提交；此前 `a78efba` 的三份真实 Provider JSON 不能证明 `a5126c8`，必须重新绑定当前 runner。
