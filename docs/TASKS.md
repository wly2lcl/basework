...
- `0b8fe97` 补齐 runner 的代理 query/fragment 脱敏、最终 stderr API key 清洗和独立验证 `WaitDelay=2s` 管道等待上界；代理 token/fragment 回归、默认/SQLite 全量与关键 race 已通过，待提交后的主 CI 回验。该 runner 变化仍需在当前候选重新生成真实 Provider artifact。
- `ceed232` 修复 `pkg/mcp/StdioTransport.Call` 的已取消 context 竞态：避免已取消请求写入，并让取消在响应同时就绪时优先。`TestStdioCallWithContextCancel` 重复 100 次、MCP 全包和默认/SQLite 全量均通过，待提交后的主 CI 回验。
