...
- 文档提交 `69b9c34` 的主 CI [35201403018](https://github.com/wly2lcl/basework/actions/runs/35201403018) 暴露 macOS `TestIndependentTestHonorsTimeoutAndBoundsOutput` 在夹具 `.basework-home` 上的清理竞态。当前修复将独立验证 HOME 移到仓库外临时目录，并增加有界清理重试；超时回归重复 50 次、默认/SQLite 全量和关键 race 均通过，待修复提交后的主 CI 回验。
