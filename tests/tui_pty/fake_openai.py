...
LOG_PATH = os.environ.get("SHIP003_FAKE_LOG", "/private/tmp/ship003/runs/fake-requests.jsonl")
...
DEFAULT_SCENARIOS = {
    ...
    "S2": {
        "steps": [
            {"tool": "bash_background",
             "args": {"command": "sh -c 'echo MARKER-BEFORE; echo $$ > /private/tmp/ship003/runs/slow-shell.pid; "
                                "SHIP003_PIDFILE=/private/tmp/ship003/runs/slow-test.pid "
    ...
    "S3": {
        "steps": [
            ...
            {"text": "已读取 main.go，并在后台启动了一次长检查。"},
        ]
    },
    "S3R": {
        "steps": [
            {"text": "收到。这是重启后的一次确认：我已看到上次会话遗留的事实。"},
        ]
    },
    ...
    "S5": {
        "steps": [
            {"tool": "write",
             "args": {"path": "/private/tmp/ship003/should-not-exist.txt", "content": "越界写入"}},
            {"text": "写入被拒绝，我没有重试。"},
        ]
    },
}
...
def scenario_for(messages):
    ...
    for m in messages:
        ...
        for token in ("[S5]", "[SE]", "[S1]", "[S2]", "[S3R]", "[S3]", "[S4]"):
            if token in text:
                return token[1:-1]
        return "S3"
    return "S3"
...
def main():
    ...
    with open(os.environ.get("SHIP003_FAKE_PORTFILE", "/private/tmp/ship003/fake.port"),
